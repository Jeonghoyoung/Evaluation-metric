...
def morphs_cjk(tup_list, num_cores=1, verbose_status=0):
    if tup_list is None:
        ...
        obj = CjkNlpModule()
        return Parallel(n_jobs=num_cores, verbose=verbose_status)(delayed(obj.split)(i) for i in tup_list)
...
def morph_nltk(tup_list, num_cores=mp.cpu_count(), verbose_status=0):
    if tup_list is None:
        ...
        return Parallel(n_jobs=num_cores, verbose=verbose_status)(delayed(obj.tokenize)(i) for i in tup_list)
...
def german_nltk_tokenizer(text_list):
    result_list = []
    for i in range(len(text_list)):
        tokenize = nltk.tokenize.WordPunctTokenizer().tokenize(text_list[i])
        result_list.append(tokenize)
    return result_list


def morphs_okt(tup_list):
    okt = Okt()
    result = [okt.morphs(text) for text in tup_list]
    return result
